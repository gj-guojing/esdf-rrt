...
class RRTSobol:
    ...
    def draw_graph(self, rnd=None):
        ...
        # 设置 x 轴的刻度间距为 1
        plt.xticks(range(self.min_rand, self.max_rand, 2))
        # 设置 y 轴的刻度间距为 1
        plt.yticks(range(self.min_rand, self.max_rand, 2))
        plt.grid(True)
        plt.pause(0.01)
...
def main(gx=7.0, gy=9.0):
    ...
    # 定义地图大小和障碍物
    map_size = [-2, 16, -2, 16]
    print("Obstacle Node Num:", len(obstacleList[0]) + len(obstacleList[1]))
    ...
    # 显示障碍物地图
    im1 = ax1.imshow(obstacle_map, cmap='gray', origin='lower', extent=map_size)
    ax1.set_xlabel('X')
    ax1.set_ylabel('Y')
    ax1.set_title('Obstacle Map')
    ax1.grid(True)
    cbar1 = fig.colorbar(im1, ax=ax1)  # 添加colorbar

    # 显示距离场
    im2 = ax2.imshow(distance_field, cmap='hot', origin='lower', extent=map_size)
    ax2.set_xlabel('X')
    ax2.set_ylabel('Y')
    ax2.set_title('Distance Field')
    ax2.grid(True)
    cbar2 = fig.colorbar(im2, ax=ax2)  # 添加colorbar
    ...
    rrt = RRTSobol(
        start=[0, 0],
        goal=[gx, gy],
        rand_area=[map_size[0], map_size[1]],
